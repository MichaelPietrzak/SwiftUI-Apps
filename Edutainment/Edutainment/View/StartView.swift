...
class Game: ObservableObject {
    ...
    func savedScoreboard() {
        if let savedScoreboard = UserDefaults.standard.data(forKey: "Scoreboard") {
            if let decoded = try? JSONDecoder().decode(Scoreboard.self, from: savedScoreboard) {
                ...
                scoreboard.bestTime = decoded.bestTime
                return
            }
        }
    }
    ...
    func saveScoreboard() {
        let saveScoreboard = Scoreboard(scores: scoreboard.scores, questions: scoreboard.questions, rightAnswers: scoreboard.rightAnswers, bestTime: scoreboard.bestTime)
